...
// Formulaire d'ajout/modification Employé
function EmployeForm({ initial, departements, onSubmit, onCancel }) {
  const [form, setForm] = useState(initial || {
    ...
    dateEntree: "",
    qualification: "",
    departement: "" // ici on stocke l'idDept en string
  });
...
export default function EmployesPage({ sidebarCollapsed }) {
  ...
  // Chargement des employés et départements
  useEffect(() => {
    setLoading(true);
    Promise.all([
      ...
      .then(([empRes, depRes]) => {
        ...
        } else if (depRes.data && Array.isArray(depRes.data.data)) {
          setDepartements(depRes.data.data);
        } else {
          setDepartements([]);
          console.warn("Format inattendu des départements :", depRes.data);
        }
        setLoading(false);
      })
      .catch(() => {
  ...
  // Options pour filtre département
  ...
  const handleAdd = (form) => {
    ...
    axios.post("http://localhost:8080/api/employes", {
      ...form,
  ...
  const handleEdit = (form) => {
    const depObj = departements.find(d => d.idDept === parseInt(form.departement));
    axios.put(`http://localhost:8080/api/employes/${form.matricule}`, {
      ...form,
      departement: depObj ? { idDept: depObj.idDept } : null
    })
      .then(res => {
            ...
            {loading ? (
              ...
              filtered.map(e => (
                <tr key={e.matricule}>
                  ...
                  <td>{e.prenom}</td>
                  <td>{e.sexe}</td>
                  <td>{e.dateNaissance?.slice(0, 10)}</td>
                  <td>{e.categorie}</td>
                  <td>{e.dateEntree?.slice(0, 10)}</td>
                  <td>{e.qualification}</td>
                  <td>{e.departement?.nomDept || ""}</td>
                  <td>
                    <button className="btn-action edit" title="Modifier" onClick={() => setEditEmploye({
                      ...e,
                      departement: e.departement?.idDept ? e.departement.idDept.toString() : ""
                    })}><MdEdit /></button>
